...
class App extends Component {
  ...
  _delPerson=async(name)=>{
    //const res = await axios.get('/api/del_person');
    //console.log(res);
    const res = await axios.delete('/api/delete_person/'+name);
    console.log(res);
  }

  _updatePerson=async()=>{
    const res = await axios.put('/api/basic_update_person');
    console.log(res);
  }

  _updatePersonParams=async(name,age,height)=>{
    console.log("updatePersonParams(name):",name);
    console.log("updatePersonParams(age):",age);
    console.log("updatePersonParams(height):",height);
    
    const res = await axios.put('/api/update_person/'+name+"&"+age+"&"+height);
    console.log(res);
  }

  _createPersonInfo=async(name,age,height)=>{
    console.log("_createPersonInfo(name):",name);
    console.log("_createPersonInfo(age):",age);
    console.log("_createPersonInfo(height):",height);
    
    const res = await axios.put('/api/create_person/'+name+"&"+age+"&"+height);
    console.log(res);
  }
  ...
  deletePersonInfo=(name)=>{
    ...
    //쿼리도 날려야함
    this._delPerson(name);

  }

  UpdatePersonInfo=(name,age,height)=>{
    alert("정보 수정(app)");
    alert("수정할값(app)"+name+age+height);

    const {personList}=this.state;
    const data = {name:name,age:age,height:height};
    const modifiedArray = personList.map(item => item.name === name? ({...item,...data}):item);
    this.setState({
      personList:modifiedArray
    })
    //this._updatePerson();
    this._updatePersonParams(name,age,height);
  }

  

  handleCreate=(name,age,height)=>{
    alert("App쪽 추가 함수");
    ...
    this._createPersonInfo(name,age,height);
  }
