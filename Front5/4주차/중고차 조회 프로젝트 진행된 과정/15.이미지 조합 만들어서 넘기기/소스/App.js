...
class App extends Component {
  ...
  allSearch=()=>{
    ...
    let img_name1=carNameInfo.코나+"_"+carColorInfo.흰색+".jpg";
    alert("1:"+img_name1);
    let carName="코나";
    let carColor="흰색";
    let img_name2=carNameInfo[carName]+"_"+carColorInfo[carColor]+".jpg";
    alert("2:"+img_name2);

    let full_img_name="./images/"+img_name2;

    global_render = carList.map( (data, i) => 
    <Car key={data.id} id={data.id} img_src={full_img_name} car_name={data.car_name}
    provider={data.provider}  color={data.color} produce_date={data.produce_date}
    receiving_date={data.receiving_date} price={data.price} accident={data.accident} 
    displacement={data.displacement} distance_driven={data.distance_driven} fuel={data.fuel} 
    transmission={data.transmission} 
