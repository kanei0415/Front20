  ...
  render() {
    ...
    return(
      <div id="car_component">
        ...
        <div><span>차량명:</span>{car_name}</div>
        <div><span>제조사:</span>{provider}</div>
        <div><span>색깔:</span>{color}</div>
        <div><span>가격:</span>{price}</div>
        <div><span>사고유무:</span>{accident}</div>
        <div><span>주행거리:</span>{distance_driven}</div>
