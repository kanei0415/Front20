...
class App extends Component{
  ...
  handleUpdate=(name,price,year)=>{
    alert("App의 수정함수");
    const data ={name:name,price:price,year:year};
    const {carList}=this.state;
  ...
  render(){
    ...
    const {carList}=this.state;
    var result = carList.map( (car,index)=><Car key={index} name={car.name} 
    price={car.price} year={car.year} 
    car_img={car.car_img} handleDelete={this.handleDelete}
